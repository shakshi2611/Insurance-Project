...
import {
  ...
  Select, MenuItem, FormControl, InputLabel, IconButton, Menu,
} from "@mui/material";
      ...
      try {
        ...
        const flattenedBrokerData = brokerResponse.data.flatMap(file =>
          file.content.map(item => {
            const commissionRateAmount = ((item.commissionRate / 100) * item.odPremium).toFixed(2);
            const rewardAmount = ((item.Reward / 100) * item.odPremium).toFixed(2);
            return {
              ...item,
...
const positiveData = brokerData
  .map(broker => {
    const matchingInsurance = insuranceData.find(insurance =>
      insurance.POLICY_NUMBER === broker.PolicyNumber &&
      insurance.INSURED_CUSTOMER_NAME === broker.cName &&
      insurance.ACTUAL_COMMISSION < broker.NetCommision
    );
    ...
    if (matchingInsurance) {
      const difference = (broker.NetCommision - matchingInsurance.ACTUAL_COMMISSION).toFixed(2); // Calculate the difference
      return {
        ...broker,
...
const negativeData = brokerData
  ...
  const filteredData = (data) =>
    selectedBank ? data.filter(item => item.p_insurerName === selectedBank) : data;
  ...
  const renderTable = (data, title) => (
    <div style={{ marginTop: "30px" }}>
      ...
      <TableContainer component={Paper} sx={{ backgroundColor: "white", boxShadow: "0px 4px 12px rgba(0, 0, 0, 0.1)" }}>
        <Table>
          <TableHead>
            <TableRow>
          ...
          <TableBody>
            {data.length > 0 ? (
              data.map((item, index) => (
                <TableRow key={index}>
                  <TableCell sx={{ color: "#000000" }}>{index + 1}</TableCell> {/* Serial Number */}
              ...
              <TableRow>
                <TableCell colSpan={12} style={{ color: "#9ca3af", textAlign: "center" }}>
                  No Data Available
                </TableCell>
              </TableRow>
            )}
  ...
  const exportToExcel = (data) => {
    // Map through the data to include the serial number and the difference
    const exportData = data.map((item, index) => ({
      ...
      OtherCommission: item.OtherCommission,
  ...
  const exportToPDF = (data) => {
    ...
    // Prepare table rows with serial number and safe access to difference
    const tableRows = data.map((item, index) => [
      index + 1, // Serial number
      item.PolicyNumber || 'N/A', // Fallback to 'N/A' if undefined
      item.p_insurerName || 'N/A', // Fallback to 'N/A' if undefined
      item.cName || 'N/A', // Fallback to 'N/A' if undefined
      item.p_type || 'N/A', // Fallback to 'N/A' if undefined
      item.NetCommision || 'N/A', // Fallback to 'N/A' if undefined
      item.difference !== undefined ? item.difference : 0, // Default to 0 if difference is undefined
    ]);
  
    // Set up the autoTable
    doc.autoTable({
      head: [tableColumn],
      body: tableRows,
      startY: 30,
  ...
  return (
    <div className="flex-1 overflow-auto relative z-10" style={{ backgroundColor: "#94a5db" }}>
      <main className="max-w-7xl mx-auto py-6 px-4 lg:px-8">
        {/* Dropdown for bank selection and export options */}
        <FormControl style={{ display: "flex", flexDirection: "row", justifyContent: "space-between", alignItems: "center", marginBottom: "20px" }}>
          <div className="container flex justify-end items-center mb-4 sm:mb-0 w-full sm:w-auto">
            <IconButton aria-label="more options" aria-controls="export-menu" aria-haspopup="true" onClick={(event) => setAnchorEl(event.currentTarget)}>
              <FileDownloadIcon style={{ color: "black" }} />
            </IconButton>

            <Menu id="export-menu" anchorEl={anchorEl} open={Boolean(anchorEl)} onClose={() => setAnchorEl(null)}>
              <MenuItem onClick={() => exportToExcel(filteredData(brokerData))}>Export to Excel</MenuItem>
              <MenuItem onClick={() => exportToPDF(filteredData(brokerData))}>Export to PDF</MenuItem>
            </Menu>
          </div>

          <FormControl sx={{ minWidth: 200 }} className="w-full sm:w-auto">
            <InputLabel id="bank-select-label" className="text-white-500 text-sm font-medium">Filter by Bank</InputLabel>
            <Select
            ...
            >
              <MenuItem value=""><span className="#cdd5ee">All Banks</span></MenuItem>
